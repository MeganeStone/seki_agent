...
def create_conversation(conn: psycopg.Connection, owner: str = "alice", conversation_id: str = "conv-1") -> None:
    chats = ChatRepository(conn)
    chats.initialize()
    chats.create_conversation(conversation_id, owner)


def test_create_and_list_pending_operation_for_owner(test_db: psycopg.Connection) -> None:
...
def test_cancel_pending_operation(test_db: psycopg.Connection) -> None:
    create_conversation(test_db)
    service = CodeOperationService(test_db)
    created = service.create_pending_from_result(
        owner_username="alice",
...
def test_confirm_delete_path_executes_and_records_message(test_db: psycopg.Connection, tmp_path: Path) -> None:
    create_conversation(test_db)
...
def test_confirm_unknown_command_does_not_execute_yet(test_db: psycopg.Connection, tmp_path: Path) -> None:
    create_conversation(test_db)
    root = tmp_path / "project"
    root.mkdir()
    execution = CodeExecutionService(allowed_roots=[root], default_root=root)
...
def test_operation_owner_isolated(test_db: psycopg.Connection) -> None:
    create_conversation(test_db)
    service = CodeOperationService(test_db)
    created = service.create_pending_from_result(
        owner_username="alice",
