...
## 22. 持久化审计表与覆盖写入确认落地状态

本轮完成第 7 节审计记录的持久化和第 9.1 节“返回 diff，等待用户确认再写入”：

持久化审计：

- 新增 `backend/app/repositories/code_audit_repository.py`，表 `code_audit_records` 记录
  用户、会话、agent、工具名、状态、目标、消息、详情和起止时间。
- 新增 `backend/app/services/code_audit_service.py`：默认 audit sink 每条记录用独立短连接
  写库（LangGraph 工具可能跨线程执行）；详情中剔除文件内容字段，避免审计表膨胀。
- `CodeExecutionService` 新增可选 `audit_sink`，运行时由 `agent_runner_factory` 和
  `CodeOperationService` 注入默认 sink；审计写入异常不会阻断工具执行；进程内
  `audit_records` 列表保留，单元测试不接 sink 时行为不变。
- 新增 `GET /api/v1/code-operations/audit`，按当前用户（可选 conversation）查询审计记录。

覆盖写入确认：

- `code_write_text_file` 覆盖既有文件时返回 `requires_confirmation`，附 unified diff 预览
  （超长会裁剪，非 UTF-8 既有文件或超过读取限制时给出说明文案）。
- 工具适配层会当场创建 pending operation；`CodeOperationService.confirm_operation` 确认后
  以 `confirmed=true` 真正写入。
- 新文件和 code agent 本次运行创建的文件可直接写入，不打断模型自我迭代。
- 前端确认卡片对 `write_text_file` 显示“覆盖写入文件”，并以 `<pre>` 渲染 diff 预览。

测试：`backend/tests/test_code_audit.py`（审计落库、内容剔除、sink 失败兜底、用户隔离 API）、
`test_code_execution_service.py`（确认前文件不变、确认后写入、agent 自建文件直写、非 UTF-8/
无变化 diff 文案）、`test_code_langchain_tool_adapter.py`（工具 → pending → 确认写入端到端）。
